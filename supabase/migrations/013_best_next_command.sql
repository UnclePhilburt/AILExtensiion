-- Lets a phone request ask its own active desktop to choose the highest-scored
-- Inbox lead. The command contains no client details; selection happens in IMPACT.
begin;
...
begin
  ...
  if action_type not in ('next','previous','best-next','call','no-answer','refused-appointment','virtual-appointment','virtual-appointment-day','virtual-appointment-slot') then raise exception 'Unknown command.'; end if;
