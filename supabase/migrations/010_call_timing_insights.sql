-- Completed-call outcomes used only for each rep's personal calling-time insight.
-- No lead names, addresses, phone numbers, or notes are stored here.
begin;
create table public.companion_call_outcomes (
  id bigint generated always as identity primary key,
  user_id uuid not null references auth.users(id) on delete cascade,
  outcome text not null check (outcome in ('no-answer','refused-appointment','virtual-appointment')),
...
create index companion_call_outcomes_user_created_idx on public.companion_call_outcomes(user_id, created_at desc);
alter table public.companion_call_outcomes enable row level security;
revoke all on public.companion_call_outcomes from anon, authenticated;
grant select, insert on public.companion_call_outcomes to authenticated;
create policy own_companion_call_outcomes on public.companion_call_outcomes for select to authenticated using (user_id = (select auth.uid()));
create policy own_companion_call_outcomes_insert on public.companion_call_outcomes for insert to authenticated with check (user_id = (select auth.uid()));
...
begin
  ...
  if action_type in ('no-answer','refused-appointment','virtual-appointment') then
    insert into public.companion_call_outcomes(user_id,outcome,request_type,local_hour)
    values (auth.uid(), action_type, left(coalesce(s.lead->>'requestType',''), 200), extract(hour from timezone('America/Chicago', now()))::smallint);
  end if;
...
do $$ begin
  if exists (select 1 from pg_extension where extname = 'pg_cron') then
    perform cron.schedule('companion-call-outcome-retention', '29 * * * *',
      $job$ delete from public.companion_call_outcomes where created_at < now() - interval '180 days' $job$);
  end if;
end $$;
commit;
