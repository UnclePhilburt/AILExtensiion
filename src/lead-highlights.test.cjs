...
test('the phone lead card shows heads-up chips above the details and hides them when empty', async()=>{
  ...
  for(const file of ['time-zone.js','pending-call.js','phone-actions.js','lead-highlights.js','lead-rules.js','settings-store.js','lead-transition.js','lead-swipe.js']) vm.runInContext(strip(read(`phone-web/public/${file}`)),context);
