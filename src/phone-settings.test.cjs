...
test('the CSS implements every display setting', () => {
  ...
  assert.match(css, /html\[data-text-size="large"\] :is\(\.workspace,\.calendarPage,\.statsPage,\.home\) \{ --text-zoom:1\.15; \}html\[data-text-size="xlarge"\] :is\(\.workspace,\.calendarPage,\.statsPage,\.home\) \{ --text-zoom:1\.3; \}/);
